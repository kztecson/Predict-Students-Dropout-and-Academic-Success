...
# Load data
students <- read.csv("C:/Users/idasi/Downloads/data.csv", sep = ";", stringsAsFactors = FALSE)

# Convert Target → Status
students$Status <- factor(students$Target)

# Basic info
dim(students)
table(students$Status)
round(prop.table(table(students$Status)), 3)
```

```{r}
ggplot(students, aes(Status)) +
  geom_bar(fill = "steelblue") +
  theme_minimal() +
  labs(title = "Counts of Student Status")
```

```{r}
summary(students$Admission.grade)
summary(students$Previous.qualification..grade.)
summary(students$Age.at.enrollment)
summary(students$Curricular.units.1st.sem..grade.)
summary(students$Curricular.units.2nd.sem..grade.)
```

```{r}
table(students$Course)
table(students$Application.mode)
table(students$Tuition.fees.up.to.date)
table(students$Scholarship.holder)
table(students$Marital.status)
```

```{r}
ggplot(students, aes(Admission.grade)) +
  geom_histogram(bins = 30) +
  theme_minimal() +
  labs(title = "Distribution of Admission Grades")
```

```{r}
ggplot(students, aes(Status, Admission.grade)) +
  geom_boxplot() +
  theme_minimal() +
  labs(title = "Admission Grade by Status")
```
